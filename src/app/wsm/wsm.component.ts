...
export class WsmComponent  implements AfterViewInit {
  ...
  onSubmit() {
    ...
    if (this.topsisForm.valid) {
      this.http.post('http://127.0.0.1:5000/waspas', criteria)
        .subscribe(response => {
  ...
  createChart() {
    ...
    const closeness = this.result.map(item => item.waspas_distance);
    const ranks = this.result.map(item => item.waspas_rank);
